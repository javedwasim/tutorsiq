...
@section('htmlheader_title')
    ...
    <?php else: ?>
    {{ trans('Admin | Location | Preference | Save') }}
    <?php endif; ?>
@endsection
...
@section('contentheader_title')
    <?php if($status != 'add'): ?>
    {{ trans('Update Preferred Location') }}
    <?php else: ?>
    <div id="element1">
        {{ trans('Save Preferred Location') }}
    </div>
    <?php endif; ?>
@endsection

@section('main-content')
    <form class="form-horizontal" id="locations" method="post" action="{{ url('teacher/location/preference/save') }}"
          enctype="multipart/form-data">
        <div class="pull-right" style="margin-top: -40px;">
            <button type="submit" class="btn btn-warning pull-right" value="save"
                    name="save" style="margin-right:5px;"><i class="fa fa-fw fa-save"></i>Save
            </button>
        </div>
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        <input type="hidden" name="status" id="status" value="<?php echo isset($status) ? $status : ''; ?>">
        <input type="hidden" name="teacher_id" id="teacher_id"
               value="<?php echo isset($teacher_id) ? $teacher_id : '';?>">
    ...
    <!-- form end -->
    @include('teachers.email-to-admin-locations')
        ...
        jQuery(document).ready(function ($) {
            ...
            $('#locations').on('submit', function (e) {
                ...
                $.ajax({
                    ...
                    url: '{{url("teacher/location/preference/save")}}',
                    type: "POST",
                    data: formData,
                    async: false,
                    beforeSend: function () {
            ...
            $( '#adminEmailLocationForm' ).on( 'submit', function(e) {
                ...
                $.ajax({
                    ...
                    url:'{{url("admin/send/email/location")}}',
                    type: "POST",
                    data: formData,
                    async: false,
                    beforeSend: function(){
