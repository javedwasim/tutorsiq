...
@section('htmlheader_title')
    ...
    <?php if($status != 'add'): ?>
    {{ trans('Admin | Teacher | Referrer | Update') }}
    <?php else: ?>
    {{ trans('Admin | Teacher | Referrer | Add') }}
    <?php endif; ?>
@endsection

@section('contentheader_title')
    <?php if($status != 'add'): ?>
    {{ trans('Update  Referrer') }}
    <?php else: ?>
    {{ trans('Add New  Referrer') }}
    <?php endif; ?>
@endsection
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="box box-info">
                    <!-- form start -->
                    <form class="form-horizontal" id="preference" method="post" action="{{ url('admin/referrer/save') }}"
                          enctype="multipart/form-data">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <input type="hidden" name="status" id="status"
                               value="<?php echo isset($status) ? $status : ''; ?>">
                        <input type="hidden" name="id" id="id"
                               value="<?php echo isset($referrer->id) ? $referrer->id : '';?>">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control" id="name"
                                                       value="<?php echo isset($referrer->name) ? $referrer->name : ''; ?>"
                                                       name="name" placeholder="Enter Referrer Name" maxlength="100"
                                                       required>
                                            </div>
                                        </div>

                                        <div class="box-footer">
                                            <a href="{{url('admin/referrers')}}" class="btn btn-warning pull-right">
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (success == 'saveandadd') {
                            ...
                            var redirect_url = '{{url("admin/referrer/add")}}';
                            toastr.success('Referrer Save Successfully!');

                        } else {

                            var redirect_url = '{{url("admin/rmessage")}}';
